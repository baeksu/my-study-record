...
### os
최적의 성능을 위해 cpu 정보를 알아야 할때 **os.cpus()** 를 이용해서 cpu 정보를 얻어올 수 있다.

### path
const path = require('path') 모듈을 사용하면 운영체제에 따른 경로 분기점을 자동으로 관리해준다.
join & resolve 에 따른 경로 차이가 있다.
- resolve 의 경우 **/** 를 절대경로로 처리한다.
```js
//'..' 을 추가하면 __dirname의 부모폴더로 올라간다.
path.join(__dirname, '..','var.js');
```

### crypto (암호화)
- hash기법 : createHash('사용할 알고리즘') ex)sha512
  이렇게하면 이제 암호화 된 값이 서버에 저장된다. 실제 '비밀번호' 를 저장하지 않는다.

- 비대칭 암호화 : rsa

### util
이전 버전의 코드들을 deprecated 로 감싸면 경고 message 를 보여줄 수 있다.
util.promisify 를 사용하면 콜백패턴을 프로미스 패턴으로 바꿔준다.
...
### 버퍼와 스트림 이해하기
100기가 동영상 파일을 저장한다고 할때, 서버 메모리가 10 기가 라고 하자 이럴때 10기가씩 10덩이러 잘라서 서버로 전송할때
버퍼와 스트림을 기술을 사용할 수 있다.
...
근데 특정 라우터에 대해서만 use 를 사용하고 싶을 수도 있는데 이떄는 매개변수 맨앞에 해당 url 을 써주면 된다.


- error를 처리해주는 미들웨어가 있다. 얘는 매개변수를 반드시 4개를 모두 다 써줘야 한다.
